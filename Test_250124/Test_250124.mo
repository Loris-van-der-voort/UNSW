...
model Test_250124
     ...
     parameter Modelica.Units.SI.Volume VRoo1=165.6348
    "Room 1 volume";
    ...
    .Buildings.ThermalZones.EnergyPlus_9_6_0.ThermalZone zon(zoneName = "GroundFloorQ1:GX112XLunchroom",redeclare replaceable package Medium = Medium,nPorts = 4) annotation(Placement(transformation(extent = {{8.0,42.0},{48.0,82.0}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Buildings.Controls.OBC.CDL.Continuous.AddParameter addPar(p = 2) annotation(Placement(transformation(extent = {{-3.929352197988612,-91.9293521979886},{3.929352197988612,-84.0706478020114}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Buildings.Controls.OBC.CDL.Continuous.PID conPID(Ti = 1800) annotation(Placement(transformation(extent = {{-82.33571969832957,-88.33571969832957},{-73.66428030167043,-79.66428030167043}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Buildings.Controls.OBC.CDL.Continuous.MultiplyByParameter gai(k = 0.1) annotation(Placement(transformation(extent = {{-4.6137609575134695,-76.39765886328532},{2.723346055785348,-69.06055184998648}},origin = {0.0,0.0},rotation = 0.0)));
    ...
    .Buildings.Fluid.HeatExchangers.Heater_T hea(m_flow_nominal = mRec_flow_nominal,dp_nominal = 200,redeclare replaceable package Medium = Medium) annotation(Placement(transformation(extent = {{78.0,-54.0},{98.0,-34.0}},origin = {0.0,0.0},rotation = 0.0)));
...
equation
    ...
    connect(conPID.y,hys.u) annotation(Line(points = {{-72.79713636200452,-84},{-72.79713636200452,-23.04788118488524},{-55.52030266687608,-23.04788118488524}},color = {0,0,127}));
    connect(conPID.y,hys2.u) annotation(Line(points = {{-72.79713636200452,-84},{-72.79713636200452,-61.96188483979109},{-56.04684448712233,-61.96188483979109}},color = {0,0,127}));
    connect(pul.y,conPID.u_s) annotation(Line(points = {{-90.04991389099325,-83.24808189892676},{-83.20286363799548,-83.24808189892676},{-83.20286363799548,-84}},color = {0,0,127}));
    ...
    connect(conPID.y,gai.u) annotation(Line(points = {{-72.79713636200452,-84},{-65.94720726555613,-84},{-65.94720726555613,-72.7291053566359},{-5.34747165884335,-72.7291053566359}},color = {0,0,127}));
    ...
    connect(addPar.y,TAirLvgSet.u2) annotation(Line(points = {{4.715222637586335,-88},{16.628867079712677,-88},{16.628867079712677,-82.24976174843864}},color = {0,0,127}));
    ...
    connect(zon.TAir,addPar.u) annotation(Line(points = {{49,80},{126,80},{126,-100},{-16,-100},{-16,-88},{-4.715222637586335,-88}},color = {0,0,127}));
    connect(zon.TAir,conPID.u_m) annotation(Line(points = {{49,80},{126,80},{126,-100},{-78,-100},{-78,-89.20286363799548}},color = {0,0,127}));
    connect(fan.port_b,hea.port_a) annotation(Line(points = {{60.68755483791662,-44.42412350932326},{68.34377741895831,-44.42412350932326},{68.34377741895831,-44},{78,-44}},color = {0,127,255}));
    connect(hea.port_b,zon.ports[4]) annotation(Line(points = {{98,-44},{104,-44},{104,36.9},{28,36.9},{28,42.9}},color = {0,127,255}));
    connect(TAirLvgSet.y,hea.TSet) annotation(Line(points = {{27.885683094250652,-79.43555774480414},{68,-79.43555774480414},{68,-30},{76,-30},{76,-36}},color = {0,0,127}));
